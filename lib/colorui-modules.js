...
const cuModules = {
  ...
  switch: tagModule(['switch', 'checkbox', 'radio']),
...
for (const m in cuModules) {
  if (cuModules[m].hasOwnProperty('type') && cuModules[m].hasOwnProperty('filter')) {
    ...
    for (const sub in cuModules[m]) {
      subModules[m].push(m + '-' + sub)
      allModules[m + '-' + sub] = cuModules[m][sub]
    }
  }
}

module.exports = { subModules, allModules }