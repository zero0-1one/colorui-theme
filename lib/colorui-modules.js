...
module.exports = function (newColors=[]) {
  const rawModules = {
  ...
  for (const { name } of newColors) {
    rawModules.color[name] = nameModule(name)
  }

  let subModules = {}
  let allModules = {}
  let moduleList = []
  for (const m in rawModules) {
    moduleList.push(m)
    if (rawModules[m].hasOwnProperty('type') && rawModules[m].hasOwnProperty('filter')) {
      allModules[m] = rawModules[m]
    } else {
      //包含子模块
      subModules[m] = []
      for (const sub in rawModules[m]) {
        let subName = m + '-' + sub
        moduleList.push(subName)
        subModules[m].push(subName)
        allModules[subName] = rawModules[m][sub]
      }
    }
  }
  return { subModules, allModules, moduleList, rawModules }
}
