...
module.exports = {
  async doAll(options) {
    ...
    if (options.lite) this.dumpLite(options, cuCss, options.newColors)
  ...
  //根据指定模块来导出使用的 css
  async dumpLite(options, cuCss, newColors) {
    let { src, dest, lite } = options
    cuCss = cuCss || fs.readFileSync(path.join(src, 'main.css'))
    let result = await postcss([postcssLite({ ...lite, newColors })]).process(cuCss, { from: undefined })
